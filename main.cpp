...
#define LED_1STRIPE 8  // Количество светодиодов в одной полоске (4 полоски последовательно) (TEST 8 == 72)
#ifdef ONE_STRIPE
...
int timeButton = 0;
int mode = 0;
#define BRIGHT_STEP 255 / LED_1STRIPE
int bright = BRIGHT_STEP;
int prog = 0;
int speed = 50;
bool up = false;
bool dn = false;
...
void loop()
{
  ...
  case MODE_PROGRAMM:
    ...
    {
    case 0:
      randomPixel(100,5,stripe,true);
      break;
    case 1:
      roundPixel(stripe.Color(255, 0, 0), 100, 1, stripe);
      roundPixel(stripe.Color(0, 255, 0), 100, 1, stripe);
      roundPixel(stripe.Color(0, 0, 255), 100, 1, stripe);
      roundPixel(stripe.Color(255, 0, 255), 100, 1, stripe);
      break;
    case 2:
      randomPixel(100, 50, stripe, false);
      break;
    case 3:
      rainbow(10, stripe);
      break;
...
void randomPixel(int wait, int rounds, Adafruit_NeoPixel &strip, bool clear)
{
  for (int i = 0; i < rounds; i++)
  {
    ...
    char colB = random();
    uint8_t num = random();
    strip.setPixelColor(num >> 5, strip.Color(colR, colG, colB));
    strip.show();
    delay(wait);
  }
  strip.clear();
