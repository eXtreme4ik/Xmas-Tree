...
volatile uint8_t packetPrevByte = 0;
...
{
  TREE_OK = 0xE0,
  TREE_ERROR_UART_CMD = 0x10,
  TREE_ERROR_UART_ARG = 0x20,
  TREE_ERROR_UART_CRC = 0x30,
  TREE_START = 0x40,
} tree_err_t;
#elif
#define GAME_MODE
...
void buildStripe(Adafruit_NeoPixel &strip, char &programma, int &firstPixelHue, uint16_t &roundcount, int sp)
{
  ...
  case 77:
  {
    for (uint16_t n = 0; n < strip.numPixels(); n++)
    {
      strip.setPixelColor(n, strip.Color(pkg.pcr, pkg.pcg, pkg.pcb));
    }
    strip.show();
  }
  break;
...
void buildTree(char &programma, int &firstPixelHue, uint16_t &roundcount, int sp)
{
  ...
  case 77:
  {
    for (int i = 0; i < 4; i++)
    {
      ...
      for (uint16_t n = 0; n < maxMusicNums; n++)
      {
        stripe[i].setPixelColor(n, stripe1.Color(pkg.pcr, pkg.pcg, pkg.pcb));
      }
      stripe[i].show();
    }
  }
...
{
  // rxdata[rxindex] = UDR0;
  // rxindex < 63 ? rxindex++:rxindex =0;
  // uartCount = 10;
  if (!rxPacketAvailable)
  {
    rxdata[rxindex] = UDR0;
    if (packetPrevByte == (uint8_t)0xCD && rxdata[rxindex] == (uint8_t)0xAB && !rxPacketstart)
    {
      rxdata[0] = 0xCD;
      rxdata[1] = 0xAB;
      rxindex = 1;
    ...
    if (rxPacketstart)
    {
      ...
      {
      uartPrint(rxindex);
      uartPrint(rxdata[rxindex]);
        rxlen = rxdata[rxindex];
      }
      rxindex++;
    }
    if (rxindex == rxlen + 5)
...
#define PREAMBLE_1 0xAB
#define PREAMBLE_2 0xCD
int uartPackageAvailable(void)
{
  // if (rxindex && !uartCount)
  // {
  //   cli();

  //   for (int i = 0; i < rxindex; i++)
  //   {
  //     if (rxdata[i] == PREAMBLE_1)
  //     {
  //       if (!rxPacketstart)
  //       {
  //         if (packetPrevByte == PREAMBLE_2)
  //         {
  //           rxPacketstart = true;
  //           startPointer = i - 1;
  //           rxlen = rxdata[startPointer + 2] + 1;
  //         }
  //       }
  //     }
  //     packetPrevByte = rxdata[i];
  //   }
  //   rxindex = 0;
  //   if (rxPacketstart)
  //   {
  //     memcpy(rxbuffer, rxdata + (startPointer + 2), rxlen);
  //     rxPacketAvailable = true;
  //     rxPacketstart = false;
  //   }
  //   sei();
...
void uartRecieveHandler(void)
{
  ...
  memcpy(rxbuffer, rxdata + 2, rxlen);
  // CRC CHECKING;
  uint8_t crcdata = rxdata[startPointer + 3 + rxlen];
  uint8_t checksum = rxdata[startPointer + 2];
  for (int i = startPointer + 3; i <= rxdata[startPointer + 2] + 3; i++)
  {
    checksum ^= rxdata[i];
  }
  // CRC CHECKING;
  if (crcdata == checksum)
  {
    ...
    rxdata[startPointer + 3 + rxlen - 1] = ((rxdata[startPointer + 2 + rxlen] & 0x0F) | cmdHandler());
    checksum = rxdata[startPointer + 2];
    for (int i = startPointer + 3; i <= rxdata[startPointer + 2] + 3; i++)
    {
      checksum ^= rxdata[i];
    }
    rxdata[startPointer + 3 + rxlen] = checksum;
    for (int i = startPointer; i <= rxlen + 3; i++)
      uartPrint(rxdata[i]);
  }
  else
  {
    rxdata[startPointer + 3 + rxlen - 1] = ((rxdata[startPointer + 2 + rxlen] & 0x0F) | TREE_ERROR_UART_CRC);
    checksum = rxdata[startPointer + 2];
    for (int i = startPointer + 3; i <= rxdata[startPointer + 2] + 3; i++)
    {
      checksum ^= rxdata[i];
...
{
  for (unsigned int i = 0; i < (sizeof(&buf) / sizeof(char)); i++)
  {
